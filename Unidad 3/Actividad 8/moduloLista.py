...
class lista():
   ...
   def buscarAgenteDni(self, dni):
      actual = self.__comienzo
      while((actual != None) and (dni != self.getDNI(actual.datos()))):
         actual = actual.sig()
      if actual != None:
         return actual.datos()
      else:
         print("No se encontro un agente con ese DNI")
   
   
   def modificarBasico(self, dni, nuevoSueldo):
      agente = self.buscarAgenteDni(dni)
      agente.setSueldoBasico(nuevoSueldo)
   
   
   def modificarImporteExtra(self, dni, nuevoImporte):
      agente = self.buscarAgenteDni(dni)
      if isinstance(agente, docenteInvestigador):
         agente.setAporteExtra(nuevoImporte)
      else:
         print("El agente no es docente investigador")
   
   
   def modificarPorcentajePorCategoria(self, dni, nuevoPorcentaje):
      agente = self.buscarAgenteDni(dni)
      if isinstance(agente, personalApoyo):
         agente.setPorcentajeCategoria(nuevoPorcentaje)
      else:
   ...
   def modificarPorcentajePorCargo(self,dni, nuevoPorcentaje):
      ...
      if isinstance(agente, docente):
         agente.setPorcentajeCargo(nuevoPorcentaje)
      else:
         print("El agente no es docente")
   
   
   def gastosSueldoPorEmpleado(self, dni):
      agente = self.buscarAgenteDni(dni)
      print(f"{agente.getSueldo():0.2f}")